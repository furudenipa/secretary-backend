...
class MobilityAgent:
    ...
    async def _search_route_info(req: schemas.MobilityRequest) -> str:
        ...
        queries = [
            ...
            f"ユーザの好み：{req.user_preferences}に基づいた施設や飲食店",
        ]
...
class MasterPlannerAgent:
    ...
    def _create_final_planning_prompt(
        ...
        1. 【移動】前の予定の場所からアクティビティの場所への移動。公共交通機関を用いる場合はどの公共交通機関を用いるかまで記載してください。
        2. 【アクティビティ】メインの活動。検索情報を元に、場所や施設の具体的な名前(固有名詞など)とそこで何をするのかを記載してください。ユーザの好みを反映させてください。ご飯の予定は連続しないようにしてください。
        3. 【移動】アクティビティの場所から次の予定の場所への移動。公共交通機関を用いる場合はどの公共交通機関を用いるかまで記載してください。
